...
def bruce(desknumber):
  desknumber = desknumber.split()
  desknumber = desknumber[0]
  seats = input().split()
  gap_index = seats.index('-')
  if desknumber in seats[:gap_index] :
   seats=gap_index+1
  if desknumber in seats[:gap_index]:
   return gap_index - seats[:gap_index].index(desknumber)
  else:
   return seats[gap_index+1:].index(desknumber)
print(bruce(input()))
